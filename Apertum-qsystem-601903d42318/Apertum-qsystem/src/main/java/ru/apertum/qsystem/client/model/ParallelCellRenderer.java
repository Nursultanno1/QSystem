...
/**
 * Ячека для параллельных.
 *
 * @author Evgeniy Egorov
 */
@SuppressWarnings({"squid:S4144", "squid:MaximumInheritanceDepth"})
public class ParallelCellRenderer extends JTextPane implements ListCellRenderer<QCustomer> {
    ...
    public Component getListCellRendererComponent(JList<? extends QCustomer> list, QCustomer value, int index, boolean isSelected, boolean cellHasFocus) {
        ...
        String str = value.getService().getInputCaption().replaceAll("<[^>]*>", "");
        if (str == null) {
            str = "";
        } else {
            str = "<u>" + str + "</u><br>" + value.getInputData();
        }
        setContentType("text/html");
        setText("");
        setText("<html>"
            + "<div style='text-align: center;'><span style='font-size:32.0pt;color:purple;'>" + textCust + "</span> "
            + "<span style='font-size:14.0pt;color:gray'> " + getPriorityName(value.getPriority().get()) + "</span>"
            + "</div>"
            ...
            + "<span style='font-size:14.0pt;color:gray'> " + str + " </span>"
            + "</div>");
    ...
    private String getPriorityName(int priority) {
        switch (priority) {
            case 0: {
                return getLocaleMessage("messages.priority.low");
            }
            case 1: {
                return getLocaleMessage("messages.priority.standart");
            }
            case 2: {
                return getLocaleMessage("messages.priority.hi");
            }
            case 3: {
                return getLocaleMessage("messages.priority.vip");
            }
            default: {
                return getLocaleMessage("messages.priority.strange");
            }
        }
    }
}
