...
/**
 * Получить описание ситуации для конкретного пользователя.
 *
...
public class RpcGetSelfSituation extends JsonRPC20 {
    ...
    public static class SelfSituation {
         ...
         * Описание себя.
         */
        public SelfSituation(LinkedList<SelfService> selfservices, QCustomer customer, LinkedList<QCustomer> parallelList, LinkedList<QCustomer> postponedList,
                             int limitRecall, int extPrior, Shadow shadow) {
    ...
    public static class SelfService {
        ...
        public SelfService(QService service, int countWait, int priority, boolean flexy, boolean flexyInvitation) {
            ...
            service.getClients().stream().forEach(cu -> {
                final CustomerShort sp = new CustomerShort(cu.getId(), cu.getFullNumber(), cu.getInputData(), cu.getWaitingMinutes());
                line.addLast(sp);
            });
        }
