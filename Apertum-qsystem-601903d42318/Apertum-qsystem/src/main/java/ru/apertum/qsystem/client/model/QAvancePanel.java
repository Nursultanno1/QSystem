...
public class QAvancePanel extends QPanel {
    ...
    public QAvancePanel(IAdviceEvent advanceEvent, Date adviceDate, final boolean enable) {
        ...
        final DateFormat formatHhMm = new SimpleDateFormat("HH:mm");
        label.setText("<html><p align=center><span style='font-size:15.0pt;color:" + (enable ? "black" : "red") + "'>"
            + ("00:00".equals(formatHhMm.format(data)) ? "24:00" : formatHhMm.format(data)) + "</span>");
        add(label);
    ...
    /**
     * Заголовок. На панеле лежит метка, в нее текст.
     *
     * @param caption текст на панель.
     */
