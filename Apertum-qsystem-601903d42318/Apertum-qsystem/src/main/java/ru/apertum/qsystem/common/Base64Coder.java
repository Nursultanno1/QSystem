...
public class Base64Coder {
     ...
     *
     * @param in   an array containing the data bytes to be encoded.
     * @param len number of bytes to process in <code>in</code>.
     * @return A character array with the Base64 encoded data.
     */
    public static char[] encode(byte[] in, int len) {
        int dataLen = (len * 4 + 2) / 3;       // output length without padding
        int len2 = ((len + 2) / 3) * 4;         // output length including padding
        char[] out = new char[len2];
        int ip = 0;
        int op = 0;
        while (ip < len) {
            int i0 = in[ip++] & 0xff;
            int i1 = ip < len ? in[ip++] & 0xff : 0;
            int i2 = ip < len ? in[ip++] & 0xff : 0;
            int o0 = i0 >>> 2;
            int io1 = ((i0 & 3) << 4) | (i1 >>> 4);
            int io2 = ((i1 & 0xf) << 2) | (i2 >>> 6);
            out[op++] = map1[o0];
            out[op++] = map1[io1];
            out[op] = op < dataLen ? map1[io2] : '=';
            op++;
            int io3 = i2 & 0x3F;
            out[op] = op < dataLen ? map1[io3] : '=';
            op++;
        }
        return out;
    }
