...
public class RpcBanList extends JsonRPC20 {
    ...
    public LinkedList<String> getBanList() { //NOSONAR
        return banCustomers;
    }
    ...
    /**
     * Эт вообще синглтн. Нужет тлько для отправки банлиста в админку.
    ...
    public void addToBanList(QCustomer customer) {
        banCustomers.add(customer.getInputData().trim());
        banSrok.put(customer.getInputData().trim(), new Date().getTime());
    }
    ...
    public boolean isBaned(QCustomer customer) {
        udo(customer.getInputData());
        return ServerProps.getInstance().getProps().getBlackTime() > 0 && banCustomers.contains(customer.getInputData().trim());
    }

    public boolean isBaned(String data) {
        udo(data);
        return data != null && ServerProps.getInstance().getProps().getBlackTime() > 0 && banCustomers.contains(data.trim());
    }

    public void deleteFromBanList(QCustomer customer) {
        banCustomers.remove(customer.getInputData().trim());
        banSrok.remove(customer.getInputData().trim());
    }
