...
/**
 * Модель для таблицы пользователей в админке.
 *
...
public class UserTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        switch (columnIndex) {
            ...
            case 2:
                String redirect = props.getAddr(userId).redirectServiceId == null || props.getAddr(userId).redirectServiceId == 0
                    ? "нет"
                    : props.getAddr(userId).redirectServiceId.toString();
                return props.getAddr(userId).getServeceName() == null ? redirect : props.getAddr(userId).getServeceName();
            case 3:
                return props.getAddr(userId).getQsize();
            case 4:
